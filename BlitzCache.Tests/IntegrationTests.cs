...
namespace BlitzCacheCore.Tests
{
    ...
    public class IntegrationTests
    {
        ...
        public void BlitzCache_Should_Use_SmartSemaphoreDictionary_For_Sync_Operations()
        {
            ...
            var result2 = cache.BlitzGet("sync_key", () => "new_value", TestHelpers.StandardTimeoutMs);
        ...
        public void BlitzUpdate_Should_Work_With_SmartSemaphoreDictionary()
        {
            ...
            var result = cache.BlitzGet("update_key", () => "fallback", TestHelpers.StandardTimeoutMs);
        ...
        public async Task Cache_Expiration_Should_Trigger_Semaphore_Cleanup()
        {
            ...
            var result = cache.BlitzGet("short_expiry", () => "expires_soon", TestHelpers.VeryShortTimeoutMs); // 50ms expiration
            ...
            await TestHelpers.WaitForStandardExpiration(); // Reduced from 100ms

            // Verify cache actually expired by trying to get new value
            var newResult = cache.BlitzGet("short_expiry", () => "new_value_after_expiry", TestHelpers.VeryShortTimeoutMs);
        ...
        public async Task Performance_Test_Smart_Dictionaries()
        {
            ...
            var iterations = (int)TestHelpers.StandardTimeoutMs;
            ...
            var testResult = await AsyncRepeater.GoWithResults(iterations, async () =>
            {
                ...
                {
                    return cache.BlitzGet($"sync_{index}_{DateTime.UtcNow.Ticks}", () => $"sync_value_{index}", TestHelpers.StandardTimeoutMs);
                }
            });
            ...
            Assert.That(duration.TotalMilliseconds, Is.LessThan(TestHelpers.LongTimeoutMs), "Should be performant");
