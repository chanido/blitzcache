...
namespace BlitzCacheCore.Tests
{
    ...
    public class IntegrationTests
    {
        ...
        public void BlitzCache_Should_Use_SmartSemaphoreDictionary_For_Sync_Operations()
        {
            ...
            var result1 = cache.BlitzGet("sync_key", () => "sync_value", TestConstants.StandardTimeoutMs);
            ...
            var result2 = cache.BlitzGet("sync_key", () => "new_value", TestConstants.StandardTimeoutMs);
        ...
        public void BlitzUpdate_Should_Work_With_SmartSemaphoreDictionary()
        {
            ...
            // Update cache
            cache.BlitzUpdate("update_key", () => "updated_value", TestConstants.StandardTimeoutMs);

            var semaphoresAfterUpdate = cache.GetSemaphoreCount();
            Console.WriteLine($"📊 Semaphores after update: {semaphoresAfterUpdate}");

            // Verify the update worked
            var result = cache.BlitzGet("update_key", () => "fallback", TestConstants.StandardTimeoutMs);
        ...
        public async Task Cache_Expiration_Should_Trigger_Semaphore_Cleanup()
        {
            ...
            var result = cache.BlitzGet("short_expiry", () => "expires_soon", TestConstants.VeryShortTimeoutMs); // 50ms expiration
            ...
            await TestDelays.WaitForStandardExpiration(); // Reduced from 100ms

            // Verify cache actually expired by trying to get new value
            var newResult = cache.BlitzGet("short_expiry", () => "new_value_after_expiry", TestConstants.VeryShortTimeoutMs);
        ...
        public async Task Performance_Test_Smart_Dictionaries()
        {
            ...
            var iterations = (int)TestConstants.StandardTimeoutMs;
            ...
            var testResult = await AsyncRepeater.GoWithResults(iterations, async () =>
            {
                ...
                {
                    return cache.BlitzGet($"sync_{index}_{DateTime.UtcNow.Ticks}", () => $"sync_value_{index}", TestConstants.StandardTimeoutMs);
                }
            });
            ...
            Assert.That(duration.TotalMilliseconds, Is.LessThan(TestConstants.LongTimeoutMs), "Should be performant");
