...
namespace BlitzCacheCore.Statistics.Memory
{
    ...
    internal sealed class ObjectGraphValueSizer : IValueSizer
    {
        ...
        // Pool to avoid allocating a new HashSet for each sizing operation. HashSet is cleared before reuse.
        private static readonly ConcurrentBag<HashSet<object>> _visitedPool = new ConcurrentBag<HashSet<object>>();
        ...
        public long GetSizeBytes(object? value)
        {
            ...
            var visited = AcquireVisitedSet();
            try
            {
                if (type.IsArray)
                {
                    return SizeOfArray((Array)value, 0, visited);
                }
                return EstimateObject(value, 0, visited);
            }
            finally
            {
                ReleaseVisitedSet(visited);
            }
        }
        ...
        private long EstimateObject(object obj, int depth, HashSet<object> visited)
        {
            ...
            bool isAdaptive = mode == SizeComputationMode.Adaptive;
            bool isAccurate = mode == SizeComputationMode.Accurate;
            int effectiveMaxDepth = _options.MaxDepth + (isAccurate ? 1 : 0);
            if (isAdaptive) effectiveMaxDepth = _options.MaxDepth; // bounded strictly below Accurate
            if (depth >= effectiveMaxDepth && !isAdaptive) return Align8(total);

            // In Adaptive mode decide per-field if we traverse deeper using a simple heuristic budget.
            int remainingObjectBudget = _options.MaxObjects - visited.Count;
            bool allowDeeper = !isAdaptive || (remainingObjectBudget > 0 && depth < _options.MaxDepth);
            foreach (var f in layout.InstanceFields)
            {
                ...
                else
                {
                    ...
                    if (fv != null)
                    {
                        ...
                        else if (allowDeeper && depth < effectiveMaxDepth)
                        {
                            ...
                            if (childLayout.HasReferenceFields || childLayout.PrecomputedShallowSize > (_options.ObjectHeaderSize + 32))
                                total += EstimateObject(fv, depth + 1, visited);
                        }
                    }
                }
            }
        ...
        private long SizeOfArray(Array arr, int depth, HashSet<object> visited)
        {
            ...
            int sampleLimit = ComputeSampleLimit(_options.Mode, _options.MaxSampledElementsPerLevel, fastCap:4, adaptiveDivisor:2, adaptiveCapMultiplier:0.75, accurateMultiplier:2);
        ...
        private bool TryCollection(object obj, Type type, int depth, HashSet<object> visited, out long size)
        {
            ...
            if (obj is IEnumerable enumerable)
            {
                ...
                int sampled = 0;
                int sampleLimit = ComputeSampleLimit(_options.Mode, _options.MaxSampledElementsPerLevel, fastCap:8, adaptiveDivisor:2, adaptiveCapMultiplier:0.75, accurateMultiplier:2, adaptiveNeedsCount: count.HasValue);
        ...
        private TypeLayoutInfo GetLayout(Type t) => _layoutCache.GetOrAdd(t, tt =>
        {
            ...
            foreach (var f in fields)
            {
                ...
                if (ft.IsValueType)
                {
                    if (IsPrimitiveLike(ft)) shallow += GetPrimitiveLikeSize(ft); else shallow += _options.FallbackStructSize;
                }
                else
                {
                    hasRef = true;
        ...
        private static HashSet<object> AcquireVisitedSet()
        {
            if (_visitedPool.TryTake(out var hs)) return hs;
            return new HashSet<object>(ReferenceEqualityComparer.Instance);
        }

        private static void ReleaseVisitedSet(HashSet<object> set)
        {
            set.Clear();
            _visitedPool.Add(set);
        }
        ...
        private static int ComputeSampleLimit(SizeComputationMode mode, int baseLimit, int fastCap, int adaptiveDivisor, double adaptiveCapMultiplier, int accurateMultiplier, bool adaptiveNeedsCount = true)
        {
            int sampleLimit = mode == SizeComputationMode.Accurate ? baseLimit * accurateMultiplier : baseLimit;
            if (mode == SizeComputationMode.Fast)
            {
                return Math.Min(sampleLimit, fastCap);
            }
            if (mode == SizeComputationMode.Adaptive && adaptiveNeedsCount)
            {
                int cap = (int)Math.Ceiling(baseLimit * adaptiveCapMultiplier);
                sampleLimit = Math.Min(sampleLimit / adaptiveDivisor, cap);
            }
            return sampleLimit;
        }
